...
6. ВАЖНО: В ответе указывай точные метаданные из раздела [Источник: ...] где ты нашел информацию.

Формат ответа (предпочтительный) - JSON:
{{"value": "найденное значение", "document": "название документа из источника", "page": "номер страницы из источника", "chunk": "номер чанка из источника"}}

Альтернативный формат - текст:
{query}: значение

Предоставленные документы (обрати внимание на метаданные в [Источник: ...]):
{context}"""
